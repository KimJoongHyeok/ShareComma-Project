...
public class HouseBoardDAO {
	...
	public HouseVO getHouseById(String id) throws SQLException {
		...
		try {
			...
			//검색결과 하우스(id,name,address,content,house_time_posted)멤버(id,name,phone,nickname,)
			...
			if(rs.next()) {
				...
				hvo.setHouseHits(null);
			 	mvo=new MemberVO(rs.getString(6),null,rs.getString(7),rs.getString(8),rs.getString(9),null,null);
				hvo.setMemberVO(null);
				ivo=new ImageVO(null,rs.getString(11),rs.getString(12),rs.getString(13),rs.getString(14));
				hvo.setImageVO(null);
	...
	public void updateHouse(HouseVO hvo) throws SQLException {
		...
		try {
			System.out.println(hvo.getHouseId()+"업데이트 시작");
			con=dataSource.getConnection();
